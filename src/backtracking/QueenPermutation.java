...
public class QueenPermutation {
    ...
    // as combination
    private static void findAsCombination(int n, boolean[] placed, int queenPlacedSoFar, int row, int col, String answer) {
        ...
        for (int i=0; i<n; i++){
            if (!placed[i]){
                placed[i] = true;
                findAsCombination(n, placed, queenPlacedSoFar+1, nextRow, nextCol, answer+"q"+(i+1)+sep);
                placed[i] = false;
            }
        }
    ...
    private static void findAs1DArray(int n, int totalBoxes, int placedSoFar, int currentItem, int[] placed) {
