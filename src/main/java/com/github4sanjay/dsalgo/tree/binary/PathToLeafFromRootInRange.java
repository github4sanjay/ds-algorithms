...
public class PathToLeafFromRootInRange {
  ...
  public static void main(String[] args) {
    var root =
        BinaryTreeUtil.create(
            new int[] {
              50, 25, 12, -1, -1, 37, 30, -1, -1, 40, -1, -1, 75, 62, 60, -1, -1, 70, -1, -1, 87,
              -1, -1
            });
  ...
  /**
   * add node to current path and node data to sum check if its leaves and sum is in range if yes
   * add to the final list
   *
   * <p>check in left child check in right child
   *
   * <p>at last remove node from current path
   */
