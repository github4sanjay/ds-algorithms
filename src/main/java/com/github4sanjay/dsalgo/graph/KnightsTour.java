...
/**
 * 1. You are given a number n, the size of a chess board. 2. You are given a row and a column, as a
 * starting point for a knight piece. 3. You are required to generate the all moves of a knight
 * starting in (row, col) such that knight visits all cells of the board exactly once. 4. Complete
 * the body of printKnightsTour function - without changing signature - to calculate and print all
 ...
 *
 * <p>Note -> When moving from (r, c) to the possible 8 options give first precedence to (r - 2, c +
 * 1) and move in clockwise manner to explore other options.
 */
public class KnightsTour {
  ...
  private static void go(int[][] chess, int row, int col, int move, ArrayList<int[][]> list) {
    ...
    if (canVisit(chess, row - 2, col + 1)) go(chess, row - 2, col + 1, move + 1, list);
    if (canVisit(chess, row - 1, col + 2)) go(chess, row - 1, col + 2, move + 1, list);
    if (canVisit(chess, row + 1, col + 2)) go(chess, row + 1, col + 2, move + 1, list);
    if (canVisit(chess, row + 2, col + 1)) go(chess, row + 2, col + 1, move + 1, list);
    if (canVisit(chess, row + 2, col - 1)) go(chess, row + 2, col - 1, move + 1, list);
    if (canVisit(chess, row + 1, col - 2)) go(chess, row + 1, col - 2, move + 1, list);
