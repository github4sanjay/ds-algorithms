...
public class LongestPathGraph {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        LongestPathGraph.findPath(graph, 0, 6)); // Answer{path=[0, 3, 2, 5, 4, 6], weight=66}
  }

  /**
   * if source and destination are equal then check if current weight is greater than previous path
   * weight other wise check in all neighbour with the source = neighbour and same destination
   */
  ...
  private static void findPath(
    ...
    for (var edge : vertex1Edges) {
      if (!visited[edge.getNbr()]) {
        ...
        findPath(
            graph, edge.getNbr(), dest, visited, currentPath, currentWeight + edge.getWt(), answer);
