...
public class SlidingWindowMax {
   ...
   * 5. Check if the element at the back of the queue is smaller than the current element , if it is so
   *    remove the element from the back of the deque, until all elements left in the deque are greater
   *    than the current element.
   * 6. Remove the element from the front of the queue if they are out of
   *    the current window.
  ...
  public static int[] findWithHelpOfMaxHeap(int[] array, int window) {
    int[] sliding = new int[array.length - window + 1];
    var queue = new PriorityQueue<Integer>(Collections.reverseOrder());
    for (int i = 0; i < window; i++) {
      queue.offer(array[i]);
    }
    sliding[0] = queue.peek();
    int j = 1;
    queue.remove(array[0]);
    for (int i = window; i < array.length; i++) {
      queue.add(array[i]);
      sliding[j] = queue.peek();
      queue.remove(array[j]);
      j++;
    }
    return sliding;
  }
