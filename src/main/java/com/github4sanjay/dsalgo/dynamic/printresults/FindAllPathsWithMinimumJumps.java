...
public class FindAllPathsWithMinimumJumps {
  ...
  private static class Node {
    int index;
    int jumpRequiredToReachEnd;
    int jumpAllowed;
    List<Integer> path;

    public Node(int index, int jumpRequiredToReachEnd, int jumpAllowed, List<Integer> path) {
      this.index = index;
      this.jumpRequiredToReachEnd = jumpRequiredToReachEnd;
  ...
  private static List<List<Integer>> find(int[] jumps) {
    ...
    while (!queue.isEmpty()) {
      var node = queue.pop();
      if (node.jumpRequiredToReachEnd == 0) {
        list.add(node.path);
      }
      for (int i = 1; i <= node.jumpAllowed && node.index + i < dp.length; i++) {
        var currentIndex = node.index + i;

        // add node whose ways = (current node ways - 1)
        if (dp[currentIndex] != null && dp[currentIndex] == node.jumpRequiredToReachEnd - 1) {
