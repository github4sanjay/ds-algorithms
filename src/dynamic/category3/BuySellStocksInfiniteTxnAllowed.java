...
public class BuySellStocksInfiniteTxnAllowed {
    ...
    public static void main(String[] args) {
        System.out.println(BuySellStocksInfiniteTxnAllowed.findAlt(new int[]{6,7,19,4,1,6,18,4})); //30
        System.out.println(BuySellStocksInfiniteTxnAllowed.findAlt(new int[]{ 100, 180, 260, 310, 40, 535, 695 })); //865
        System.out.println(BuySellStocksInfiniteTxnAllowed.findAlt(new int[]{ 100, 10, 260, 310, 40, 535, 640 })); //900
        System.out.println(BuySellStocksInfiniteTxnAllowed.findAlt(new int[]{ 10,9, 8,7 })); // 0
        System.out.println(BuySellStocksInfiniteTxnAllowed.findAlt(new int[]{ 100,200,300,400 })); // 300
    }
    ...
    private static int find(int[] stockPrices) {
        ...
        while (i < stockPrices.length) {
            ...
            if (buy && stockPrices[i] < stockPrices[i+1] && stockPrices[i] < stockPrices[i-1] ){
                ...
                sell = true;
                buy = false;
            } else if (sell && stockPrices[i] > stockPrices[i+1] && stockPrices[i-1] < stockPrices[i]){
                profit += stockPrices[i] - boughtAt;
                buy = true;
                sell = false;
            }
