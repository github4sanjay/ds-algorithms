...
public class BuySellStocksKTxnAllowed {
    ...
    public static void main(String[] args) {
        System.out.println(BuySellStocksKTxnAllowed.findAlt(new int[]{11,6 ,7 ,19,4 ,1 ,6 ,18,4}, 2));
    }
        ...
        for (int i = 0; i<= k; i++){
            for (int j = 0; j< stockPrices.length; j++){
                if (i ==0 || j ==0){
                    ...
                    for (int t=0; t<j; t++){ // 0 to j-1 is i-1 transaction, ith transaction can be between current j and any of 0 to j-1
