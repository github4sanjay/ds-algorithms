...
public class PaintHouse {
    ...
    private static int findMinCostBruteForce(int[][] colorCost, int houseIndex, int previousColorChosen, int cost) {
        ...
        for (int i=0; i< colorCost[0].length; i++){
            if (i != previousColorChosen || previousColorChosen == -1){
                int currentCost = findMinCostBruteForce(colorCost, houseIndex+1, i, cost+colorCost[houseIndex][i]);
                if (currentCost < min){
                    min = currentCost;
                }
            }
