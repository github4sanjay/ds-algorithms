 ...
 * At every step, it slides the pattern by the max of the slides suggested by the two heuristics. So it uses best of the two heuristics at every step.
...
public class BoyerMoore {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(check("The Boyer Moore algorithm does preprocessing for the same reason", "preprocessing"));
        System.out.println(check("The Boyer Moore algorithm does preprocessing for the same reason", "Boyer"));
    }
    ...
    public static boolean check(String word, String pattern) {
        ...
        while (i<word.length()){ // starts comparing from end
            if (word.charAt(i) == pattern.charAt(window)){
                ...
                var position = table.getOrDefault(word.charAt(i), pattern.length()); // if not found take default as pattern length
                i = i+position;
            }
        }
        return false;
    ...
    /**
     * table[i] = pattern.length() - i - 1 for all character
     * except last character table[i] = pattern.length()
     */
