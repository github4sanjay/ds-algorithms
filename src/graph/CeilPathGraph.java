...
public class CeilPathGraph {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(CeilPathGraph.findPath(graph, 0, 6, 30)); // Answer{path=[0, 1, 2, 5, 4, 6], weight=36}
    }

    /**
     * if source and destination are equal then check
     *  1. if current weight is greater than ceil
     *  2. and smaller than previous path weight
    ...
    private static void findPath(Graph graph, int src, int dest, int ceil, boolean[] visited, List<Integer> currentPath, int currentWeight, Answer answer) {
        ...
        for (var edge : vertex1Edges){
            if (!visited[edge.getNbr()]){
                ...
                findPath(graph, edge.getNbr(), dest, ceil, visited, currentPath, currentWeight+edge.getWt(), answer);
