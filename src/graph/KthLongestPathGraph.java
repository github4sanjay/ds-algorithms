...
public class KthLongestPathGraph {
    ...
    private static void findPath(Graph graph, int src, int dest, int k, boolean[] visited, List<Integer> currentPath, int currentWeight, PriorityQueue<Answer> priorityQueue) {
        ...
        for (var edge : vertex1Edges){
            if (!visited[edge.getNbr()]){
                ...
                findPath(graph, edge.getNbr(), dest, k, visited, currentPath, currentWeight+edge.getWt(), priorityQueue);
