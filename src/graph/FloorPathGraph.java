...
public class FloorPathGraph {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(FloorPathGraph.findPath(graph, 0, 6, 30)); // Answer{path=[0, 1, 2, 5, 6], weight=28}
    }

    /**
     * if source and destination are equal then
     *  1. check if current weight is smaller than floor
     *  2. and greater than previous path weight
    ...
    private static void findPath(Graph graph, int src, int dest, int floor, boolean[] visited, List<Integer> currentPath, int currentWeight, Answer answer) {
        ...
        for (var edge : vertex1Edges){
            if (!visited[edge.getNbr()]){
                ...
                findPath(graph, edge.getNbr(), dest, floor, visited, currentPath, currentWeight+edge.getWt(), answer);
